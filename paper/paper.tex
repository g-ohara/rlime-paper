...
\begin{figure}[tbp]
  ...
  \def\hhh{2.1}
  \def\hhhh{2.8}
  \begin{tikzpicture}[scale=0.9,nodestyle/.style={rectangle,node font=\normalsize\itshape,auto}]
    \node[nodestyle] (post-hoc) at (0,0){post-hoc};
    \node[nodestyle] (dependent) at (-\w,-\hh){model-dependent};
    \node[nodestyle] (agnostic) at (\w,-\hh){\textbf{model-agnostic}};
    \node[nodestyle] (global) at (0,-\hhhh){global};
    \node[nodestyle] (local) at (\ww,-\hhhh){\textbf{local}};
    \draw(post-hoc) -- (0,-\h);
...
\ifnum\mode=1
  ...
  the expected number of samples is bounded by
  % \begin{equation}
  %   \mathcal{O}\left[H_{\epsilon}\log\frac{H_\epsilon}{\delta}\right],
  % \end{equation}
  % where $H_{\epsilon}$ is sample complexity of the problem
  % \cite{kaufmann2013information}.
  \begin{equation}
    \mathcal{O}_{\mathrm{MAB}[B\cdot m,B]}=
    \mathcal{O}\left[\frac{Bm}{\epsilon^2}\log\frac{Bm}{\epsilon^2\delta}\right].
  \end{equation}

  % \subsubsection{Complexity of Each Trial}
  Each trial of KL-LUCB algorithm consists of sampling a perturbation vector $z$,
  getting output $f(z)$, and updating the linear classifier $g$.
  If we sample a perturbation vector from a multivariate normal distribution
  using Cholesky decomposition in advance ($\mathcal{O}(2^m\cdot m^3)$),
  we can sample $z$ in $\mathcal{O}(m^2)$.
  If we use random forest as the black-box model,
  the complexity of getting output is $\mathcal{O}(1)$.
  If we use logistic regression as the linear classifier and
  update it by stochastic gradient descent (SGD)~\cite{robbins1951stochastic},
  the computational complexity of updating $g$ for each trial is
  $\mathcal{O}(m)$.
  Then the computational complexity of each trial is $\mathcal{O}(m^2)$.

  % \subsubsection{Total Complexity}
  Overall, the complexity of R-LIME is bounded by
  \begin{equation}
    \mathcal{O}\left[
      2^m\cdot m^3+\frac{Bm^4}{\epsilon^2}\log\frac{Bm}{\epsilon^2\delta}
      \right],
  \end{equation}
  which is very sensitive to the number of features $m$.
\fi
...
\section{Conclusion}
We argued that the interpreting the outputs of black-box models is crucial
for their application in sensitive domains,
and identified challenges in existing methods for local model-agnostic post-hoc
