...
\def\mode{0} % 0: Graduation thesis, 1: ICPR paper

\ifnum\mode=0
	\documentclass{article}
	\usepackage{prmlthesis}
\else
...
% BELLA\cite{radulovic2023bella} addresses the non-optimization of the approximation region,
% a drawback of LIME,
% by learning a local linear classifier $g$
% using a subset $Z'$ of the dataset $Z$
% instead of perturbed samples.
...
\ifnum\mode=1
	...
	On the other hand, the number of queries required for Anchor and R-LIME
	is $|\mathcal{D}|$,
\fi
...
\begin{figure}[tbp]
	\centering
	\includegraphics[width=0.55\textwidth]{experiments/exp2/box_plot}
...
\section{Challenges and Future Perspectives}
 {%
  ...
  \begin{figure}[t]
	  ...
	  \begin{subfigure}[t]{\imgwidth}
		  \centering
		  \includegraphics[scale=\scale]{newlime}
		  \caption{R-LIME for balanced label distribution.}\label{fig:balanced}
	  \end{subfigure}
	  \begin{subfigure}[t]{\imgwidth}
		  \centering
		  \includegraphics[scale=\scale]{newlime_for_imbalanced_data}
