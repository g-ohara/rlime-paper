...
\ifnum\mode=1
  ...
  % \subsubsection{Updating the Linear Classifier}
  % For Anchor, each trial is composed of sampling a perturbation vector $z$
  % and getting output $f(z)$.
  % But for R-LIME, a trial
  % includes additionally updating the linear classifier $g$ by $z$ and $f(z)$.
  % If we use support vector machine (SVM) as the linear classifier and
  % update it by stochastic gradient descent (SGD)~\cite{robbins1951stochastic},
  % the computational complexity of updating $g$ for each trial is $\mathcal{O}(m^2)$.
\fi
