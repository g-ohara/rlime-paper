...
\else
  ...
  \begin{abstract}
    ...
    We demonstrate the effectiveness of the proposed method through
    qualitative and quantitative comparisons with the existing method
...
The dataset contains information on 9549 prisoners released from
North Carolina prisons between July 1, 1979, and June 30, 1980.
The dataset includes 19 items such as race (\emph{Race}), gender (\emph{Gender}),
...
was split into training data (7639 instances) and test data (955 instances).
...
that attributes such as having a prior offenses (\emph{Priors}),
being served for a long time in prison (\emph{MonthsServed}),
...
On the other hand,
attributes like older age (\emph{Age}), being married (\emph{Married}),
...
Furthermore, the explanations provide their accuracy and coverage,
allowing users to evaluate reliability of the explanations.
...
where $\tau$ is the threshold for the accuracy of the linear classifier,
...
these results suggest that the deviation is not significant in practice.
