...
\ifnum\mode=0
  ...
  \usepackage{luatexja}
  \usepackage{src/style/prmlthesis}
...
\graphicspath{{./src/}{../python/data/}{src/experiments/}}
...
We modify Anchor's definition of accuracy in \cref{eq:anchor_def_prec}
as follows:
\begin{equation}
...
under the modified definition of accuracy in \cref{eq:def_prec},
...
To satisfy the constraint imposed by \cref{eq:const_prec},
a rule $A$ needs to meet the following condition:
\begin{equation}
  \Prec_{l}(A,\delta)>\tau,\label{eq:stop_condition}
\end{equation}
...
includes rules satisfying \cref{eq:stop_condition},
...
\ifnum\mode=1
  ...
  If we use logistic regression as the linear classifier
  and update it by stochastic gradient descent (SGD) \cite{robbins1951stochastic},
...
Under the same settings as in \cref{sec:exp_setting},
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.5\textwidth]{src/experiments/exp2/box_plot}
...
\ifnum\mode=0
  \bibliographystyle{src/style/prml}
\else
